...
var init_StreamPad = __esm({
  "src/StreamPad.ts"() {
    "use strict";
    init_eventemitter3();
    StreamPad = class extends import_index.default {
      /**
      ...
      async publish(data) {
        ...
        try {
          if (this.dataType === "audio" || this.dataType === "video") {
            ...
            const tracks = this.dataType === "audio" ? data.getAudioTracks() : data.getVideoTracks();
            if (tracks.length === 0) {
       ...
       */
      async subscribe(element) {
        if (this.direction === "source") {
          console.log(`\u{1F4E1} Setting up source pad subscription for ${this.dataType} pad: ${this.name}`);
          this.setSubscribeState("subscribing");
          this.on("stream-received", (stream) => {
            console.log(`\u{1F4E1} Source pad ${this.name} received stream, attaching to element`);
            element.srcObject = stream;
            element.play().catch((error) => {
              console.warn(`\u{1F4E1} Autoplay failed for ${this.dataType} element:`, error);
            });
            this.setSubscribeState("subscribed");
          });
          if (this.currentStream) {
            console.log(`\u{1F4E1} Source pad ${this.name} already has stream, attaching immediately`);
            ...
            console.log(`\u{1F4E1} Source pad ${this.name} waiting for stream...`);
            this.setSubscribeState("subscribed");
          }
          return;
        }
...
init_eventemitter3();
init_StreamPad();
var WorkflowNode = class extends import_index.default {
  /**
  ...
  constructor(id, type) {
    ...
    this.publishedStreams = /* @__PURE__ */ new Map();
   ...
   * @returns {IStreamPad | null} The matching pad or null if not found
   */
  getTriggerSourcePadByName(name) {
...
// src/WorkflowEngine.ts
var WorkflowEngine = class extends import_index.default {
  /**
   ...
   */
  createNodeFromParticipant(participant) {
    const nodeId = this.extractNodeIdFromParticipant(participant);
    ...
    if (identity === "human") {
      for (const [nodeId, node] of this.nodes.entries()) {
        if (node.type === "human" || node.type === "Human") {
          console.log(`\u{1F50D} Mapped "human" identity to human node: ${nodeId}`);
          return nodeId;
        }
      }
      console.log(`\u{1F50D} No human node found for "human" identity`);
      return null;
    }
